<template>
  ...
  <section class="container_grid">
    <DoughnutChart type=1 title="Finance 綜合平均分數" :data="5" />
    <DoughnutChart type=3 title="平均分數" :data="6" />
  </section>
  <p class="other_subtitle">構面概覽</p>
  <section class="container_grid">
    <BarChartHorizontal class="h-52"  type=2 title="未來投資能力" :data="1" />
    <BarChartHorizontal class="h-52"  type=2 title="經營成長" :data="2" />
    <BarChartHorizontal class="h-52"  type=2 title="財務結構" :data="3" />
  </section>
</template>
<script lang="ts">
